                                                 ...
                                                 -- trad/translations.lua file. Add a new language  --
                                                 -- block (e.g. de = {...}) alongside en and fr.    --
                                                 --   The english version helps with translation.   --
                                                 -----------------------------------------------------
...
ATT.Questsui                        = require("system/questsui")
ATT.Search                          = require("system/search")
...
local categories = {
  ...
  Questsui = {
      -- Base-game main jobs, one flag per leaf so the left menu mirrors the weapons UX.
      "showmain_prologue", "showmain_act1", "showmain_interlude",
      "showmain_act2",
      "showmain_evelyn", "showmain_alt", "showmain_hellman", "showmain_hanako",
      "showmain_nocturne", "showmain_devil", "showmain_sun", "showmain_star", "showmain_secret",
      "showmain_epilogue",
      -- Base-game side jobs (Quêtes secondaires) by quest giver / theme
      "showsec_johnny", "showsec_judy", "showsec_panam", "showsec_river", "showsec_kerry",
      "showsec_lifepaths", "showsec_aldecaldos", "showsec_brendan", "showsec_claire",
      "showsec_coach", "showsec_delamain", "showsec_peralez", "showsec_joshua", "showsec_nix",
      "showsec_regina", "showsec_wilson", "showsec_viktor", "showsec_zen",
      "showsec_vehicles", "showsec_apartments", "showsec_misc",
      -- Other base-game buckets
      "showside", "showgig",
      -- Phantom Liberty main jobs (tarot kings)
      "showmain_pl_phantom", "showmain_pl_kow", "showmain_pl_kos",
      "showmain_pl_koc", "showmain_pl_kop", "showmain_pl_epilogue",
      -- PL side jobs
      "showsec_pl_reyes", "showsec_pl_hands", "showsec_pl_misc",
      -- Other PL buckets
      "showside_liberty", "showgig_liberty",
  },
...
--   1. langue active   (ex: fr)
--   2. anglais         (langue de référence, toujours complète)
--   3. clé brute       (utile en dev pour repérer les manquantes)
-- Garantie : ne renvoie JAMAIS nil, même si une langue est partiellement traduite
-- ou si selectedLanguage pointe sur une locale absente.
function ATT.T(key)
  local lang = ATT.selectedLanguage or "en"
  local active = ATT.translations[lang]
  if active and active[key] ~= nil then
      return active[key]
  end
  local en = ATT.translations.en
...
-- Alias local pour rester compatible avec le code existant qui appelle T(key).
local T = ATT.T

local translationstests = require("trad/translations").descriptions
-----------------------------------------------------------
...
function ATT.LoadSettings()
  ...
  if file then
      ...
      if success and settings then
          ...
          -- Backward compat: old saves used "trad" as French language code
          if ATT.selectedLanguage == "trad" then
              ATT.selectedLanguage = "fr"
          end
...
-----------------------------------------------------------
-- Détection de la langue du jeu (CP2077)
-----------------------------------------------------------
-- Renvoie le code langue de l'UI du jeu ("en-us", "fr-fr", "de-de", ...)
-- ou nil si Game.* n'est pas encore prêt (avant onInit).
function ATT.DetectGameLanguage()
    local result
...
-- Aligne ATT.selectedLanguage sur la langue du jeu :
--   "en-us" / "en-gb" → "en"
--   tout le reste     → "fr"
-- À appeler après onOverlayOpen (Game.* doit être disponible).
-- Retourne true si la détection a réussi.
function ATT.AutoSelectLanguage()
    local code = ATT.DetectGameLanguage()
    if not code then return false end
    if code:lower():match("^en") then
        ATT.selectedLanguage = "en"
        showEnglish = true
    else
        ATT.selectedLanguage = "fr"
...
registerForEvent("onOverlayOpen", function()
    ...
    ATT.Questsui.UpdateInventoryCache()
end)
...
function setCategory(module, name)
  if module then
      module.currentCategory = name
  end
  ATT.currentCategory = name  -- 🔹 garde aussi la catégorie globalement
end
registerForEvent("onDraw", function()

  if isOverlayOpen then 
    -- if ImGui.Begin(T("ATT_TiTLE")) then
    if ImGui.Begin("2.5 DEVMOD") then
    ...
    ----------------------------------------------------------
      if ImGui.BeginChild("Child Window 4", 0, 0, true) then
        ...
        if ImGui.BeginChild("Child Window 9", 0.3 * childWindow9Width, 0, true) then
          ...
          ImGui.Text("Catégorie : " .. (ATT.currentCategory or ""))
          ...
          if ATT.currentProfile == "nomad" then
            ...
            if ATT.selectedGame == "Phantom Liberty" then
              ...
              ImGui.Spacing()
                ATT.DrawUIQuestsPhantomLiberty()
            end
          end
        ...
        if not ATT.general.showGeneralUI  and not ATT.changelog.showchangelogUI and not ATT.information.showinformationUI then
          ...
          if ImGui.BeginChild("Child Window 10", 1 * childWindow10Width, 0, false) then
            ImGui.Spacing()
            -- Global search short-circuits the normal category panes:
            -- when the bar in the header has any text, we render a flat
            -- result list instead of the per-profile/per-game stats UI.
            if ATT.Search.IsActive() then
              ATT.Search.DrawResults()
              ImGui.EndChild()
            else
            if ATT.currentProfile == "nomad" then
              ...
              ATT.Questsui.ownedTotalQuestsui,                ATT.Questsui.totalTotalQuestsui                 = ATT.Questsui.count()
              ATT.Questsui.ownedTotalQuestsuiliberty,         ATT.Questsui.totalTotalQuestsuiliberty          = ATT.Questsui.countliberty()
          ...
          elseif ATT.currentProfile == "att2" then
            ...
            local ownedQuestsuiTotal              = ATT.Questsui.ownedTotalQuestsui or 0
            local totalQuestsuiTotal              = ATT.Questsui.totalTotalQuestsui or 0
            local ownedQuestsuilibertyTotal       = ATT.Questsui.ownedTotalQuestsuiliberty or 0
            local totalQuestsuilibertyTotal       = ATT.Questsui.totalTotalQuestsuiliberty or 0
            ...
            if ImGui.BeginChild("Child Window 5", 1 * childWindow5Width, 0.6 * childWindow5Height, true) then
              if ATT.selectedGame == "Cyberpunk 2077" then
                ...
                ATT.DrawUIQuestslistCyberpunk2077()
              end
              if ATT.selectedGame == "Phantom Liberty" then
                ...
                ATT.DrawUIQuestslistPhantomLiberty()
              end
            ImGui.EndChild()
            end
              ...
              --debut armes
                if ATT.Weaponsui.showhandguns then
                  if ATT.Weaponsui.selectedName and ATT.Weaponsui.groupedWeapons.Handguns[ATT.Weaponsui.selectedName] then
                    ...
                    local acquisitionText = showEnglish and (data.acquisitionen or "") or (data.acquisitionfr or "")
                    local vendorText = showEnglish and (data.vendoren or "") or (data.vendorenfr or "")
              ...
              --debut cyberwares
                if ATT.Cyberwaresui.showArms then
                  if ATT.Cyberwaresui.selectedName and ATT.Cyberwaresui.groupedCyberwares.Arms[ATT.Cyberwaresui.selectedName] then
                    ...
                    local acquisitionText = showEnglish and data.acquisitionen or data.acquisitionfr
              ...
              if ATT.Vehicules.showcar or ATT.Vehicules.showmoto then
                if ATT.Vehicules.selectedVehicle then
                    ...
                    local displayName = showEnglish and (translation and translation.en or vehicleData.name)
                                                      or (translation and translation.fr or vehicleData.name)
                    local additionalText = showEnglish and (translation and translation.addionalen)
                                                       or (translation and translation.addionalfr)
                ...
                --                              or (ATT.Vehicules.vehicleTranslations[vehicleData.id] and ATT.Vehicules.vehicleTranslations[vehicleData.id].addionalfr)
                --       if displayName then
                --         if ImGui.TreeNode(T("ATT_Obtention")) then
                --           ImGui.TextWrapped(displayName or "")
                --           ImGui.TreePop()
          ...
          ImGui.EndChild()
            end -- closes Search.IsActive() if/else
          end
        end
        end
...
function ATT.DrawUIHeader()
    ...
    ImGui.SameLine()
    -- Push search bar to the right edge of the header.
    local avail = ImGui.GetContentRegionAvail()
    if avail and avail > 280 then
        ImGui.SetCursorPosX(ImGui.GetCursorPosX() + (avail - 280))
    end
    ATT.Search.DrawBar(240)
    ImGui.Spacing()
    ImGui.Separator()
end
...
function ATT.DrawUIQuestsCyberpunk2077()
  ATT.Questsui.count()
  ATT.Questsui.uiquests()
end
function ATT.DrawUIQuestsPhantomLiberty()
  ATT.Questsui.countliberty()
  ATT.Questsui.uiquestsliberty()
end
...
function ATT.DrawUIQuestslistCyberpunk2077()
  local Q = ATT.Questsui
  local main = Q.groupedQuests.main

  -- Prologue is the one leaf whose displayed list depends on the active profile.
  if Q.showmain_prologue  then Q.displayQuestGroup(Q.GetFilteredPrologue()) end
  if Q.showmain_act1      then Q.displayQuestGroup(main.act1)      end
  if Q.showmain_interlude then Q.displayQuestGroup(main.interlude) end
  if Q.showmain_act2      then Q.displayQuestGroup(main.act2.root)    end
  if Q.showmain_evelyn    then Q.displayQuestGroup(main.act2.evelyn)  end
  if Q.showmain_alt       then Q.displayQuestGroup(main.act2.alt)     end
  if Q.showmain_hellman   then Q.displayQuestGroup(main.act2.hellman) end
  if Q.showmain_hanako    then Q.displayQuestGroup(main.act2.hanako)  end
  if Q.showmain_nocturne  then Q.displayQuestGroup(main.act3.nocturne) end
  if Q.showmain_devil     then Q.displayQuestGroup(main.act3.devil)    end
  if Q.showmain_sun       then Q.displayQuestGroup(main.act3.sun)      end
  if Q.showmain_star      then Q.displayQuestGroup(main.act3.star)     end
  if Q.showmain_secret    then Q.displayQuestGroup(main.act3.secret)   end
  if Q.showmain_epilogue  then Q.displayQuestGroup(main.epilogue)      end

  local sec = Q.groupedQuests.secondary
  if Q.showsec_johnny     then Q.displayQuestGroup(sec.johnny)     end
  if Q.showsec_judy       then Q.displayQuestGroup(sec.judy)       end
  if Q.showsec_panam      then Q.displayQuestGroup(sec.panam)      end
  if Q.showsec_river      then Q.displayQuestGroup(sec.river)      end
  if Q.showsec_kerry      then Q.displayQuestGroup(sec.kerry)      end
  if Q.showsec_lifepaths  then Q.displayQuestGroup(sec.lifepaths)  end
  if Q.showsec_aldecaldos then Q.displayQuestGroup(sec.aldecaldos) end
  if Q.showsec_brendan    then Q.displayQuestGroup(sec.brendan)    end
  if Q.showsec_claire     then Q.displayQuestGroup(sec.claire)     end
  if Q.showsec_coach      then Q.displayQuestGroup(sec.coach_fred) end
  if Q.showsec_delamain   then Q.displayQuestGroup(sec.delamain)   end
  if Q.showsec_peralez    then Q.displayQuestGroup(sec.peralez)    end
  if Q.showsec_joshua     then Q.displayQuestGroup(sec.joshua)     end
  if Q.showsec_nix        then Q.displayQuestGroup(sec.nix)        end
  if Q.showsec_regina     then Q.displayQuestGroup(sec.regina)     end
  if Q.showsec_wilson     then Q.displayQuestGroup(sec.wilson)     end
  if Q.showsec_viktor     then Q.displayQuestGroup(sec.viktor)     end
  if Q.showsec_zen        then Q.displayQuestGroup(sec.zen)        end
  if Q.showsec_vehicles   then Q.displayQuestGroup(sec.vehicles)   end
  if Q.showsec_apartments then Q.displayQuestGroup(sec.apartments) end
  if Q.showsec_misc       then Q.displayQuestGroup(sec.misc)       end

  if Q.showside then Q.displayQuestGroup(Q.groupedQuests.side) end
  if Q.showgig  then Q.displayQuestGroup(Q.groupedQuests.gig)  end
end
function ATT.DrawUIQuestslistPhantomLiberty()
  local Q = ATT.Questsui
  local pl = Q.groupedQuests.main_liberty

  if Q.showmain_pl_phantom  then Q.displayQuestGroup(pl.phantom_liberty) end
  if Q.showmain_pl_kow      then Q.displayQuestGroup(pl.king_wands)      end
  if Q.showmain_pl_kos      then Q.displayQuestGroup(pl.king_swords)     end
  if Q.showmain_pl_koc      then Q.displayQuestGroup(pl.king_cups)       end
  if Q.showmain_pl_kop      then Q.displayQuestGroup(pl.king_pentacles)  end
  if Q.showmain_pl_epilogue then Q.displayQuestGroup(pl.epilogue)        end

  local secpl = Q.groupedQuests.secondary_liberty
  if Q.showsec_pl_reyes then Q.displayQuestGroup(secpl.reyes) end
  if Q.showsec_pl_hands then Q.displayQuestGroup(secpl.hands) end
  if Q.showsec_pl_misc  then Q.displayQuestGroup(secpl.misc)  end

  if Q.showside_liberty then Q.displayQuestGroup(Q.groupedQuests.side_liberty) end
  if Q.showgig_liberty  then Q.displayQuestGroup(Q.groupedQuests.gig_liberty)  end
end
